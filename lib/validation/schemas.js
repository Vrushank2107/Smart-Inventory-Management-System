...
export const CartItemSchema = z.object({
  ...
  // Be tolerant of production payloads where `quantity` might arrive as a string
...
export const CalculateRequestSchema = z.object({
  ...
  // Default so missing `userType` doesn't hard-fail validation
...
export function validateRequest(schema, data) {
  try {
    ...
    // IMPORTANT: this function must never throw in production; errors here currently cause 500s.
    if (error instanceof z.ZodError) {
      ...
      try {
        ...
        // Fallback for any unexpected error shapes
        return { success: false, error: 'Validation failed' };
      }
    }
